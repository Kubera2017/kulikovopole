...
export class YaMap implements OnInit, OnChanges {
  ...
  @Output() public boundsChange: EventEmitter<mapTypes.MapBoundsChangeEvent> = new EventEmitter<mapTypes.MapBoundsChangeEvent>();
  ...
  private _initMapInstance(el: HTMLElement) {
    ...
    this._mapsWrapper.disableScrollZoom();
    this._mapsWrapper.getBounds().then(bounds => {
      const value = { newBounds: bounds } as mapTypes.MapBoundsChangeEvent;
      this.boundsChange.emit(value);
    });
  ...
  private _handleMapMouseEvents() {
    ...
    mapEvents.forEach((e: Event) => {
      const s = this._mapsWrapper.subscribeToMapEvent<{ latLng: any }>(e.name).subscribe(
        (event: any) => {
          const newBounds = event.get('newBounds');
          const value = { newBounds: newBounds } as mapTypes.MapBoundsChangeEvent;
          e.emitter.emit(value);
        });
      this._observableSubscriptions.push(s);
    });
